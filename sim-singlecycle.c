/* sim-singlecycle.c - educational single-cycle timing simulator */
...
/*
 * This file implements an educational single-cycle timing model. It is based
 * on sim-safe, so it preserves precise functional execution and instruction
 * checks, but it assigns exactly one simulated cycle to each committed
 * instruction. It does not model physical datapath delays or clock period.
 */
...
/* educational single-cycle timing counter */
...
{
  opt_reg_header(odb,
"sim-singlecycle: This simulator implements an educational single-cycle\n"
"timing model. It executes instructions functionally like sim-safe, and\n"
"charges exactly one simulated cycle per committed instruction. It does not\n"
"model physical datapath delays or clock period.\n"
		 );
...
sim_main(void)
{
  ...
  fprintf(stderr, "sim: ** starting single-cycle functional simulation **\n");
  ...
  while (TRUE)
    {
      ...
      /* single-cycle model: every committed instruction takes one cycle */
      sim_cycle++;
